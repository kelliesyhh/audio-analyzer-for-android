...
import github.bewantbe.depressionanalysis.Recording;
...
class SamplingLoop extends Thread {
	...
	private boolean boolStartRecording = Recording.boolStartRecording;
...
//      Log.i(TAG, "delay = " + delay);
	...
	public void run() {
		...
		System.out.println("boolStartRecording " + boolStartRecording);
...
//      FPSCounter fpsCounter = new FPSCounter("SamplingLoop::run()");
			...
			try {
				...
				while (isRunning) {
					...
					if (recorderMonitor.updateState(numOfReadShort)) {  // performed a check
						...
						if (bSaveWavLoop)
							wavSecRemain = wavWriter.secondsLeft();
						System.out.println("OVERRUN CHECK PERFORMED");
					}
					if (bSaveWavLoop) {
					...
					if (stft.nElemSpectrumAmp() >= analyzerParam.nFFTAverage) {
						...
						System.out.println("Spectrogram updated");
						//          fpsCounter.inc();
	...
	private byte[] short2byte(short[] sData) {
		...
		//System.out.println("short2byte complete");
		return bytes;
	}
